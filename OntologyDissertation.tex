...
DL ontologies are notably utilised to create knowledge bases (KB) in the biomedical sciences but have also found wider use in artificial intelligence research and other industries \cite[29]{handbookIntro2003}.\par
...
Some may argue that this ontology manifests the `EquivalenceIsDifference' antipattern where an equivalent class is erroneously designated as a subclass \cite[2]{antipatterns2009}.
...
Roussay, et al. identify three categories of antipattern: logical, non-logical and guidelines \cite[1]{antipatterns2009}.\par
...
The maximal detail ontology offers an interesting example of the `OnlynessIsLoneliness' logical antipattern \cite[1]{antipatterns2009}.
...
Two roles, {R1} and \textit{R2}, are disjoint from one another.
...
\newline
We can observe the reasoner alerting the reasoner to the fact that the ontology is now inconsistent as a \textit{View} with a discoloured depiction of Mount Fuji is erroneously placed into the Coloured\_Mount\_Fuji class.\par
The reasoner highlights a simple example of a logical antipattern.
Cushion\_Pine\_at\_Pine is simultaneously placed into two disjoint TBoxes:
\begin{quote}
  1. X $\bigsqcup$ Y \\
  2. X(a) \textbf{and} Y(a) // inconsistent from 1
\end{quote}
This is an example of an `AndIsOr' antipattern \cite[1]{antipatterns2009}.
...
\centerline{\textit{Figure 8: A comparison of the three ontologies - closed, open and MVP}}
\hfill \break
...
The \textit{Views} ontologies have raised some of the potential issues with utilising OWL for an unusual case study.
Yet none of these issues have proved to be insurmountable barriers.\par
It is true that the option to implement closed-world assumption is not offered as a feature of OWL languages but this does not preclude the ontology engineer from simulating a closed-world assumption.
...
In this case study, it was not necessary to include the MVP ontology in §3.4.\par
The OWL standard does not have to change to accomodate closed-world assumption as it is already possible to describe a closed-world within its framework.
...
As evidenced by the detailed open-world assumption's attempt to implement a pseudo-open-world assumption, the role of subsumption is integral to OWL.
Although every singleton class is subsumed by owl:Thing there are no additional restrictions being inherited by each class.
Dedicating the entirity of the TBox level to isolating each of the individuals in the ontology wastes half of the tools at the ontology engineer's disposal to build their knowledge base.
The abuse of reflexive properties in this ontology has also been discussed in §3.5.
None of the reasoners currently bundled with Protégé can identify poor ontology design: while it may prove a challenge to implement, many integrated development environments (IDE) offer intelligent code completion and recommendations to keep developers on track.
...
Ultimately, OWL succeeds in providing a versatile KR system without encroaching on the engineer's ability to describe atypical KBs.
The case study does suggest that some features may be lacking from Protégé's extend toolset.
...
It provides an intuitive interface through the use of a layer of abstraction between the engineer and the ontology artefacts.
...
Lastly, the print\_output() uses a while loop to remove whitespace and print all lines that match the search term.\par
...
The implementation of this parser gives a glimpse into the structure of ontology engineering as a field.
...
\begin{quote}
  Moreover, we need to know more about what people find easy or hard to handle... In normal commonsense situations, when reading a geography book, for instance, the ability to handle disjunctions (say) seems to be quite limited. \cite[89]{expressiveness1987}
\end{quote}
...
It had previously been established that naive reasoners - those without a firm grasp of logical rules - have to rely on some form of mental modelling to make an inference, with inclusive disjunction proving particularly challenging to envision \cite[552]{usability2014}.
...
While an individual may stuggle to parse some inferences quickly and correctly on first glance, in most DL use-cases they will have time for consideration and evaluation.\par
...
Legibility is both a surmountable problem and one which primarily appears in the less fundamental strata of this ecosystem.
...
Developers need to implement software that can interact with OWL in its abstract form and parse this abstract form to produce a serialized ontology in a given OWL language. 
Engineers use these tools provided by the developers to produce ontologies for stakeholders.
As touched upon in the previous section, there is a delicate balance to be struck between these parties.\par
The role of W3 as a forum to coordinate the active development of OWL is invaluable... %citation
% mention W3 as mediator?
% have to ensure accessible to all three
% discuss nature of abstraction and error-handling
