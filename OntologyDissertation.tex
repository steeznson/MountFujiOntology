...
There are only seven out of the thirty-six \textit{Views} that do not feature people.\footnote{Hokusai's work is primarily concerned with articulating mankind's place in nature; a popular theme in nineteenth century art, known as Romanticism.}
...
\newline
We can observe the reasoner alerting the reasoner to the fact that the ontology is now inconsistent as a \textit{View} with a discoloured depiction of Mount Fuji is erroneously placed into the Coloured\_Mount\_Fuji class.
...
The detailed open-world ontology succeeds in providing a detailed description of the \textit{Views} despite its modal handicap, however the extravagant lengths it goes to to achieve this disqualify it from serious consideration.
It falls prey to the same charges of editorialisation as the maximal detail ontology does whilst being twice the size as an ontology artefact.
...
The results are ABoxes that look syntactically very close to TBoxes: `mountFujiForeground(South\_Wind\_Clear\_Sky, South\_Wind\_Clear\_Sky)' is not substantively different from, `Mount\_Fuji\_Foreground(South\_Wind\_Clear\_Sky)'.
