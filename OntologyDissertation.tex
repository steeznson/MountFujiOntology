...
\end{quote}
Modern DL languages have pursued this notion.\footnote{Bachman \& Levanesque discuss how this approach is ``relaxing the standards of correctness'' (81). Although this is the case we shall observe later that the \textit{applications} of software make many allowances for error-handling.}

\begin{quote}
  Description logic languages are then viewed as the core of knowledge representation systems, considering both the structure of a DL knowledge base and its associated reasoning services (Baader, 2003, pp.5).
\end{quote}
% knowledge buses, KR systems are explicit structures that channel knowledge (B\&L79)
%(OWN THOUGHT) need to be able to recognise the _shape_ of a proposition
%Brachman 1979 transitive identity attributions. IS-A relationships.
Quillian's semantic networks (1967) and Minsky's frame systems (1981) both pioneered network structures that represented sets of individuals and their relationships. Post-1985 description logics began under the name ``terminological systems'' before being known as ``concept languages'' (Baader, 2003, pp.6-7).
\begin{quote}
  The first basic step for construction is provided by the disjointed alphabets of symbols that are used to denote atomic concepts, designated by unary predicate symbols; the latter are used to express relationships between concepts (Baader, 2003, pp.11).
\end{quote}

\begin{quote}
  Two possible solutions to intractability problem: first is to push the computational barrier as far back as possible, the second is to relax our standards of correctnes (B\&L81)
\end{quote}
% second solution, comparison with error handling OWN IDEA
...
% 1993 Patel-Schneider and Swartout propose LISP-like syntax
\begin{quote}
