...
A similar indescrepansey to the Notre Dame example becomes logically impossible to account for.
...
Ultimately the Pellet reasoner proved more useful, although the inclusion of multiple reasoners in Protégé software is an invaluable asset to ontology engineers who may have to model a wide variety of ontologies.\par
...
The reasoner output from the case-study demonstrates several OWL antipatterns, design practices that often lead to incoherent ontologies.
Roussay, et. al identify three categories of antipattern: logical, non-logical and guidelines \cite[1]{antipatterns2009}.
Identification of logical antipatterns is most urgent as a logical inconsistencies guarantee ontology inconsistencies.
Non-logical antipatterns concern the erroneous addition of restrictions, relationships or classes.
...
The guidelines are antipatterns that one must avoid in order to maintain an efficient and legible ontology.\par
The reasoner output for our MVP ontology in §3.4.2 is a simple example of a logical antipattern.
Cushion\_Pine\_at\_Pine is simultaneously placed into two disjoint TBoxes:
\begin{quote}
  1. X(),Y() \\
  2. X $\bigsqcup$ Y \\
  3. X(a) \& Y(a) // inconsistent, from 2
\end{quote}
