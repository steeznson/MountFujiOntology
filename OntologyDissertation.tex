...
Levesque \& Brachman (1987) significantly found that first-order logic (FOL) not suitable for describing these sorts of complex relationships.
...
\begin{quote}
  The expressive power of FOL determines not so much what can be said, but what can be left unsaid... In other words, the database retrieval version of the question asks about the structures in the database itself, and not about what these structures represent. (Levesque \& Brachman, 1987, 82)
\end{quote}
...
\begin{quote}
  The model of a domain in DL is embedded in a knowledge base. Roles of TBox and ABox motivated by the need to distinguish general knowledge about the domain of interest from specific knowledge about individuals characterising a specific world/situation under construction. (Baader, 2003, 27)
\end{quote}
...
OWL does not adhere to the Unique Name Assumption (UNA) that prevents names from co-referring to individuals, as a consequence an individual could for example be referred to with multiple names.
...
They can be used to describe the relationships between individuals or a single individuals attributes.
Finally classes (TBox) are used to create taxonomies (hierarchies of sets) that individuals are instances of.
Every class must be subsumed by a primitive class called ``owl:Thing'' but aside from this restriction the engineer is given a blank canvas to populate with their classes.
...
As the name might suggest there are thirty-six prints and each one depicts Mount Fuji in either in the foreground or background.
...
It is restricted to an exact cardinality and the set of works contains specific individuals.
...
In particular ABox predicates can slow down the reasoner's deductions considerably.
...
This provides scope for several different ontology formulations that would otherwise not be possible.\par
