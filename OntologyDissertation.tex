...
% REWRITE FROM HERE AFTER CORRECTING ONTOLOGIES
...
The reflexive characteristic denotes properties that necessarily relate individuals to themselves: in this case the class owl:Thing can only fully represent the closed \textit{Views of Mount Fuji} by having an exact relationship to those individuals.
...
\newline
\centerline{\includegraphics[scale=0.3]{ClosedViewsClass}}\\
...
While it may be more parsimonious to merge the classes and apply both properties to owl:Thing, in this case it is more descriptive to restrict the application of axioms to one per class.
...
Curiously, the closed-world assumption ought to make reasoning more tractable, however in this case the Pellet reasoner is designed to reason efficiently over an ontology with the open-world assumption.
As this ontology forces the closed-world assumption with the two reflexive properties ( and containsSpecificIndividuals), the reasoner is having to factor in many additional steps when handling the thirty-six individuals.
...
\newline
\newline
\centerline{\includegraphics[scale=0.3]{ClosedDisjointUnion}}\\
