...
Following a brief exploration of OWL's roots in description logics, this article logics seeks to test the OWL2 standard with an unusual case study.
...
The first two ontologies experiment with OWL's \textbf{open-world assumption} default while attempting to provide a detailed description of the \textbf{closed-world} of the \textit{Views}.
...
The OWL2 standard the lastest version of OWL and is recommended by the World Wide Web Consortium (W3) for ontology engineering \cite[22]{introduction2011}.
...
There are different types of components involved - such as screws, pedals and tires - and some of these components may be interchangeable with other components in their group while others are absolutely essential to the design.\par
...
This case poses a particular challenge for OWL as, unlike a manufacturing plant, a full description of the \textit{Views} includes the restriction that there can only ever be the specific thirty-six individuals being represented in the ontology.
...
The first ontology artefact attempts to create the KB in as much detail as possible while overriding OWL's defaults.
The second accepts OWL's default settings buttries to maintain the same level of description as the first ontology.
...
The resulting ontology is lean and substantially more efficient compared to its counterparts.\par
...
A critical look is taken at OWL syntax and the broader concerns of the OWL ecosystem, and additional recommendations are offered in these areas respectively.
...
\newline
This property acts as a white-list for the reasoner for valid individuals in the ontology. The `exactly' keyword is essential to stipulate this condition.
...
There is a difference in the functionality of these two classes and the order of the generality being established is important: first it is established that there are thirty-six owl:Things and then more specifically the white-list of allowed things is introduced.
...
This flexibility stems from the OWL open-world assumption, the reasoner could think that the class can only have one individual, until it becomes apparent that there is another, and then tries to accommodate this relationship by postulating that the singleton classes have merged.\par
...
It falls prey to the same charges of editorialising as the maximal detail ontology does whilst being twice the size as an ontology artefact.
...
The OWL standard does not have to change to accommodate closed-world assumption as it is already possible to describe a closed-world within its framework.
...
Dedicating the entirety of the TBox level to isolating each of the individuals in the ontology wastes half of the tools at the ontology engineer's disposal to build their knowledge base.
...
The case study does suggest that some features may be lacking from Protégé's extend tool-set.
...
The license itself, BSD 2-Clause, is very permissive and allows ``redistribution and use in source and binary forms, with or without modification,'' provided that the original license is bundled with any distribution.\footnote{GitHub: https://github.com/protegeproject/protege/blob/master/license.txt - accessed 10.04.18}
...
As discussed in §4.1, plugins can offer a partial solution to challenging case studies that require unusual or unweildy ontologies.\par
...
While FaCT++ lacks support for key constraints and some datatypes,\footnote{University of Manchester: http://owl.cs.manchester.ac.uk/tools/fact/ - accessed 10.04.18} Pellet seeks to implement complete OWL expressivity.\footnote{SemanticWeb Wiki: http://semanticweb.org/wiki/Pellet - accessed 10.04.18}
This disparity could lead the same ontology to be judged to have two different coherence evalutations depending on what reasoner is used.
In this case the standards of correctness are entirely dependent on implementation.
In the long term a standardised reasoner implementation could help clear up this ambiguity.

\chapter{Conclusion}
The \textit{Thirty-six Views of Mount Fuji} series posed a challenge to OWL.
This article has covered issues of plugin availability, ontology engineer editorialisation, the enforcement of proper OWL procedure, syntax concerns and reasoner standardisation.
...
The enforcement of proper OWL procedure and reasoner standardisation are most pressing recommendations.
While plugin availability is also important, the Protégé team are already encouraging community development through supporting plugins as a feature, their use of a permissive license and hosting the source files for the application publically on GitHub.
...
In particular the open-world ontology flouts conventional OWL practice eggregiously in its use of ABoxes as surrogate TBoxes and this ought to have led to a warning from the reasoner.
...
OWL technologies are still in their infancy but an increased focus on consistent software implementation could pay back dividends as their use becomes more widespread. 
